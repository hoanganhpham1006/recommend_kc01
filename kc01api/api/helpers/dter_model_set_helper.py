...
def processing(dataset_name, model_name):
    if not os.path.isdir(os.path.join(settings.BASE_DIR, "api/models", dataset_name, model_name)):
        ...
        dter_model[dataset_name] = tf.keras.models.load_model(model_path, custom_objects={"recall20": recall20}, compile=False)
        dter_model[dataset_name].compile(optimizer='adam',
        ...
        dter_model[dataset_name].predict(test_input)
        print("Restored successfully DTER Model! " + dataset_name + "/" + model_name)
        map_url_trainid[dataset_name] = pickle.load(open(data_path + '/map_url_trainid_' + subfix + '.pkl', 'rb'))
