...
def processing(list_url):
    list_url = [map_url_trainid[url[:-1]] for url in list_url.split() if url[:-1] in map_url_trainid]
    print(list_url)
    ...
    results = softmax[0].argsort()[-21:][::-1]
    print(results)
    list_rec = [remap_info[nid] for nid in list(results) if nid != 0]
    return list_rec[:20]